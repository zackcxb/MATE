...
- **测试模式（VLLMBackend）**：通过 OpenAI API 通信，从 logprobs 提取 token_ids，主要用于流程验证。提取策略按优先级：
  1. 读取 `choice.token_ids`（需自定义 vLLM 扩展参数 `return_token_ids=True`，仅vLLM v0.10+支持）
  2. 从 `logprobs.content[].token` 经 `tokenizer.convert_tokens_to_ids()` 词表反查恢复 ID（标准 vLLM 均支持，已验证与方案 1 产出完全一致。但由于有tokenize错位风险，仅作为低版本vLLM备选方案）
  3. 回退：`tokenizer.encode(content)`（不推荐，BPE 重编码可能与生成时 token 边界不同）
...
### VLLMBackend token_ids 提取改进

标准 vLLM 0.8.x 不支持在 `choice` 级别直接返回 `token_ids` 数组。改为从 logprobs 逐 token 提取, vLLM 0.10以上版本仍可使用return_token_ids参数直接获取token_ids：

- 方法：`tokenizer.convert_tokens_to_ids(logprobs.content[i].token)` — 词表反查，非 BPE 重编码
- 验证：roundtrip 测试确认 `convert_ids_to_tokens` 与 `convert_tokens_to_ids` 严格互逆
- `decode(token_ids, skip_special_tokens=True) == response_text` 成立
- 新增 `VLLMBackend.with_tokenizer()` 工厂方法，支持传入本地 tokenizer
