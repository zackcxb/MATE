...
同时，本轮 brainstorming 不应只对照 `swe_agent`。新增的 Slime 研究说明，另一条强基线是“完整序列 `tokens` 合同 + 训练侧零 re-tokenize”。这意味着我们至少要比较三类路线：

1. `prompt_ids` side-channel
2. `prompt_ids` first direct generation（`swe_agent` 风格）
3. full-sequence `tokens` first contract（Slime 风格）
...
1. `swe_agent`：`prompt_ids` 作为真实生成输入，并做 replay/alignment 校验
2. Slime：存储完整序列 `tokens`，训练侧直接消费，不做重新 tokenize
...
下一窗口的具体输出与退出条件，以 `docs/plans/2026-03-16-bgrpo-v03-impl-plan.md` 中的 decision gates、work packages 与 exit criteria 为准，避免在 handoff design 与 pre-plan 中重复维护两套出口定义。
